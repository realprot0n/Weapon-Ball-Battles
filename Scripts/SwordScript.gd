...
@export var swordBase: Node2D
@export var swordCollision: SwordHitbox
...
func _ready():
	...
	swordBase.rotate(deg_to_rad(rng.randf_range(0.0, 360.0)))
...
func _physics_process(delta):
	if canSpinWeapon:
		swordBase.rotate(deg_to_rad(delta*60*5*swordCollision.direction))
